...
def clean_channel_url(url):
    """Remove subpaths like /videos, /shorts to get base channel URL"""
    parsed = urlparse(url)
    path = parsed.path.lower()
    # If path ends with /videos, /shorts, /playlists etc., strip them
    for sub in ['/videos', '/shorts', '/playlists', '/community', '/channels', '/about']:
...
def extract_info():
    ...
    # If it's a channel URL with extra path, clean it
    ...
    try:
        ...
        if 'formats' in info and info.get('id'):
            ...
            vid = {
                ...
                'formats': formats,
                'average_rating': info.get('average_rating','N/A'),
        ...
        # Force channel type if URL matches channel pattern
        if is_channel_url(url):
            ...
            channel_info = {
                ...
                'total_videos': info.get('total_videos', len(entries)),
        ...
        # If not forced channel, check for playlist
        if not entries:
            # fallback channel
            return jsonify({
                'type': 'channel',
        ...
        # Playlist (when URL is a playlist)
        pl = {
            'type': 'playlist',
            'title': info.get('title',''),
